...
EOWARN
  ...
  tee_user=${COUCHDB_USER}
  tee_password=${COUCHDB_PASSWORD}

  # create the config file in /app/.config.js
  tee_config=/app/.config.js
  tee_config_template=/app/.config.js.template
  tee_config_template_content=$(cat ${tee_config_template})
  tee_config_content=${tee_config_template_content//<DB_USERNAME>/${tee_user}}
  tee_config_content=${tee_config_content//<DB_PASSWORD>/${tee_password}}
  tee_config_content=${tee_config_content//<DB_URL>/http:\/\/localhost:5984}

  echo "${tee_config_content}" > ${tee_config}

	if [ "$(id -u)" = '0' ]; then
		exec setpriv --reuid=couchdb --regid=couchdb --clear-groups "/opt/couchdb/bin/couchdb" & cd /app && npm run start
	fi

  exec "/opt/couchdb/bin/couchdb" & cd /app && npm run start
